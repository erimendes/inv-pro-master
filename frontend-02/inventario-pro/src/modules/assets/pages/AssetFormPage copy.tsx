...
export default function AssetFormPage() {
  ...
  try {
    const payload = {
      ...
      ipRede: form.ipRede?.trim() || null,
      oQueRoda: form.oQueRoda?.trim() || null,
      sistOper: form.sistOper?.trim() || null,
      cpu: form.cpu?.trim() || null,
      ram: form.ram?.trim() || null,
      discoFisico:
        form.discoFisico?.trim() || null,

      status: form.status,
      emUso: form.emUso ?? true,

      dataCompra: form.dataCompra
        ? new Date(form.dataCompra)
        : null,

      valor: Number(form.valor || 0),

      isVirtualizado:
        form.isVirtualizado ?? false,

      hyperVName:
        form.hyperVName?.trim() || null,

      hostFisicoId:
        form.hostFisicoId || null,
  ...
  return (
    <div className="mx-auto max-w-7xl p-6">
      ...
      <div className="mb-8">
        ...
        className="rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-slate-300 transition hover:bg-slate-800"
      >
        ← Voltar
      </button>
      {/* BOTÕES */}
